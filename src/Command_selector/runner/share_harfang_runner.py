...
class ShareHarfangRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.buffer_size):
                ...
                obs, share_obs, rewards, dones, infos, action_mask = self.envs.step(actions)

                # end_time = time.time()  # 记录结束时间
                # elapsed_time = end_time - start_time  # 计算时间差
                # print(f"Function execution took {elapsed_time} seconds")
                data = obs, share_obs, actions, rewards, dones, action_log_probs, values, rnn_states_actor, rnn_states_critic, action_mask
    ...
    def warmup(self):
        # reset env
        obs, share_obs, action_mask = self.envs.reset()
        ...
        self.buffer.action_mask[0] = action_mask.copy()
    ...
    def collect(self, step):
        ...
        values, actions, action_log_probs, rnn_states_actor, rnn_states_critic \
            = self.policy.get_actions(np.concatenate(self.buffer.share_obs[step]),
                                      ...
                                      np.concatenate(self.buffer.action_mask[step])
                                      )
    ...
    def insert(self, data: List[np.ndarray]):
        obs, share_obs, actions, rewards, dones, action_log_probs, values, rnn_states_actor, rnn_states_critic, action_mask = data
        ...
        self.buffer.insert(obs, share_obs, actions, rewards, masks, action_log_probs, values, \
                           rnn_states_actor, rnn_states_critic, action_mask, active_masks=active_masks)
    ...
    def render2(self):
        ...
        for episode in range(10):
            # self.envs.render(mode='txt', filepath=f'{self.run_dir}/{self.experiment_name}_{episode}.txt.acmi')
            render_episode_rewards = 0

            render_obs, render_share_obs, render_action_mask = self.envs.reset()
            render_masks = np.ones((1, *self.buffer.masks.shape[2:]), dtype=np.float32)
            render_rnn_states = np.zeros((1, *self.buffer.rnn_states_actor.shape[2:]), dtype=np.float32)
            ...
            while True:
                ...
                render_actions, render_rnn_states = self.policy.act(np.concatenate(render_obs),
                                                                    np.concatenate(render_rnn_states),
                                                                    np.concatenate(render_masks),
                                                                    np.concatenate(render_action_mask),
                                                                    deterministic=True)
                render_actions = np.expand_dims(_t2n(render_actions), axis=0)
                render_rnn_states = np.expand_dims(_t2n(render_rnn_states), axis=0)

                # Obser reward and next obs
                render_obs, render_share_obs, render_rewards, render_dones, render_infos, render_action_mask = self.envs.step(render_actions)
    ...
    def modify_attn_weights(self, attn_weights_np):
        ...
        for i in range(num_rows):
            ...
            # 在第i个位置插入1
            modified_row = np.insert(row, i, 1)
