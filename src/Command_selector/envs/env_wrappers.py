...
class ShareDummyVecEnv(DummyVecEnv, ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, share_obs, rews, dones, infos, action_mask = map(list, zip(*results))
        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], share_obs[i], action_mask[i] = self.envs[i].reset()
            elif isinstance(done, (list, tuple, np.ndarray)):
                if np.all(done):
                    obs[i], share_obs[i], action_mask[i] = self.envs[i].reset()
            elif isinstance(done, dict):
                ...
                raise NotImplementedError("Unexpected type of done!")
        self.actions = None
        return self._flatten(obs), self._flatten(share_obs), self._flatten(rews), self._flatten(dones), np.array(infos), self._flatten(action_mask)

    def reset(self):
        results = [env.reset() for env in self.envs]
        obs, share_obs, action_mask = map(np.array, zip(*results))
...
def shareworker(remote: Connection, parent_remote: Connection, env_fn_wrappers):
    ...
    """
    def step_env(env, action):
        obs, share_obs, reward, done, info, action_mask = env.step(action)
        if 'bool' in done.__class__.__name__:
            if done:
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                remote.send(CloudpickleWrapper((getattr(envs[0], "num_agents", envs[0].ally_num))))
            else:
                raise NotImplementedError
    except KeyboardInterrupt:
        print('SubprocVecEnv worker: got KeyboardInterrupt')
...
class ShareSubprocVecEnv(SubprocVecEnv, ShareVecEnv):
    ...
    def step_wait(self):
        ...
        results = self._flatten_series(results) # [[tuple] * in_series] * nremotes => [tuple] * nenvs
        self.waiting = False
        obs, share_obs, rewards, dones, infos, action_mask = zip(*results)
        return self._flatten(obs), self._flatten(share_obs), self._flatten(rewards), self._flatten(dones), np.array(infos), self._flatten(action_mask)
    ...
    def reset(self):
        ...
        obs, share_obs, action_mask = zip(*results)
        return self._flatten(obs), self._flatten(share_obs), self._flatten(action_mask)
