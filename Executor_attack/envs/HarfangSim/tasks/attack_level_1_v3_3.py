...
class TaskConfig(object):
    def __init__(self):
        ...
        self.hit_prob = 0.6  # 在攻击范围内被击中的概率
        self.hyper_feature_safe_distance = 50  # 飞机之间的碰撞距离为50m

        # 奖励参数相关,全部设为正值
        self.hyper_reward_out_bound = 60  # 出界惩罚
        self.hyper_reward_step_punish = 0.02  # 步数惩罚
        self.hyper_reward_distance_punish = 2  # 安全距离惩罚

        # 角度优势距离
        self.hyper_feature_oppo_TA = pi / 6  # 表示敌方的尾追角，就是敌方是远离我还是靠近我
        self.hyper_feature_oppo_AO1 = pi / 6
        self.hyper_feature_oppo_AO2 = pi / 12
        self.hyper_feature_oppo_AO3 = pi / 30

        # 攻击下逼近指定目标的奖励
        self.hyper_feature_close_range = 2500  # 逼近范围
        self.hyper_feature_close_angle = pi / 6  # 逼近角度
        self.hyper_feature_remote_angle = pi / 2  # 远离角度
...
class MultiCombatTask(object):
    def __init__(self):
        ...
        self.last_target_dis = [0] * self.ally_num
    ...
    def load_observation_space(self):
        """ 指定观测空间、共享观测空间"""
        self.obs_length = 17 + (self.ally_num - 1) * 15 + self.enemy_num * 17
    ...
    def get_obs(self, agent_id, ally_states, enemy_states):
        ...
        得到自己的观测信息
        自身信息：
            位置、速度、三个姿态角，姿态角解算，当前总速度、健康值、指定的目标
        队友信息：
            相对位置、速度、AA，ATA,HA,距离、是否和我目标相同的标志
        敌方信息：
            相对位置、速度、AA、ATA、HA，距离，组别编码
        """
        own_feat = np.zeros((14, ), dtype=np.float32)  # 自身信息
        ally_feats = np.zeros((self.ally_num - 1, 14), dtype=np.float32)  # 队友信息
        enemy_feats = np.zeros((self.enemy_num, 14), dtype=np.float32)  # 敌方信息
        ...
        if own_state["health_level"] > 0:
            ...
            own_feat[0:14] = np.array([norm_own_x, norm_own_y, norm_own_z, norm_own_vx, norm_own_vy,
                                       norm_own_vz, pitch_sin, pitch_cos, yaw_sin, yaw_cos, roll_sin,
                                       roll_cos, norm_speed, health_level])
            ...
            for enemy_id in range(self.enemy_num):
                ...
                if enemy_id_state["health_level"] > 0:
                    ...
                    enemy_feats[enemy_id, 0:14] = np.array([norm_enemy_relative_x, norm_enemy_relative_y,
    ...
    def get_reward(self, agent_id, ally_states, enemy_states):
        ...
        if own_state["health_level"] > 0:
            ...
            reward_step_punish = self.get_reward_step_punish(agent_id)  # 步数惩罚

            reward_close_target = self.get_close_target_reward(agent_id, own_state, enemy_states)  # 逼近指定目标的奖励
            reward_angle_advantage = self.get_reward_angle_advantage(agent_id, own_state, enemy_states)  # 与目标组别的角度优势

            reward_attack = self.get_reward_attack(agent_id, own_state, ally_states, enemy_states)  # 攻击或被攻击的奖励
            # # #
            # print("reward_out_bound:", reward_out_bound,
            #       "reward_safe_distance:", reward_safe_distance,
            #       "reward_step_punish:", reward_step_punish,
            #       "reward_close_target:", reward_close_target,
            #       "reward_dis_angle_advantage:", reward_angle_advantage,
            #       "reward_attack:", reward_attack)

            external_reward = (1.0 * reward_out_bound + 1.0 * reward_safe_distance + 1.0 * reward_step_punish +
                                1.0 * reward_angle_advantage + 0.0 * reward_attack)
            internal_reward = (1.0 * reward_close_target)

            own_reward = external_reward + internal_reward

        return own_reward
    def get_reward_angle_advantage(self, agent_id, own_state, enemy_states):
        """得到与目标的角度优势"""
        reward_angle = 0
        own_pos_vector = own_state["position"]
        own_move_vector = own_state["move_vector"]
        ...
        for enemy_id in self.command_groups_running[own_target_group]:
            ...
            if enemy_id_state["health_level"] > 0:
                ...
                # # 因为要加入元命令相关奖励，所以整体逼近奖励加在元命令那部分
                # if own_R <= self.task_hyper.hyper_feature_close_range:
                #     # 距离较小时，角度优势给予奖励
                #     if abs(own_TA) <= self.task_hyper.hyper_feature_oppo_TA:
                #         # 表示敌方背对我
                #         if abs(own_AO) <= self.task_hyper.hyper_feature_oppo_AO3:
                #             reward_angle += 0.2
                #         elif abs(own_AO) <= self.task_hyper.hyper_feature_oppo_AO2:
                #             reward_angle += 0.01
                #         elif abs(own_AO) <= self.task_hyper.hyper_feature_oppo_AO1:
                #             reward_angle += 0.005
                #         else:
                #             reward_angle += 0
                #
                #     if abs(enemy_TA) <= self.task_hyper.hyper_feature_oppo_TA:
                #         # 表示我方背对敌方
                #         if abs(enemy_AO) <= self.task_hyper.hyper_feature_oppo_AO3:
                #             reward_angle -= 0.3
                #         elif abs(enemy_AO) <= self.task_hyper.hyper_feature_oppo_AO2:
                #             reward_angle -= 0.015
                #         elif abs(enemy_AO) <= self.task_hyper.hyper_feature_oppo_AO1:
                #             reward_angle -= 0.008
                #         else:
                #             reward_angle -= 0
                ...
                if own_R <= self.task_hyper.hyper_feature_close_range:
                    # 距离较小时，角度优势给予奖励
                    angle_advantage = (pi - own_AO - own_TA) / pi * 0.02
            reward_angle += angle_advantage

        return reward_angle

    def get_done(self, agent_id, ally_states, enemy_states):
        """
        判断该智能体是否参与
        """
        done = False
        ...
        if own_state["health_level"] == 0:
            ...
            # 血量不为0时
            done = self.done_out_bound(agent_id, own_state)  # 出界终止
    ...
    def enemy_out_range(self, enemy_states):
        """敌方飞出界终止"""
        for enemy_id in range(self.enemy_num):
            enemy_state = enemy_states[enemy_id]
            if enemy_state["health_level"] > 0:
    ...
    def reset_machine(self):
        ...
        # 定义敌方飞机的基准位置（单位：米）
        enemy_pairs = {
            1: {'base_position': (4500, 5000), 'formation_offset': [(-500, 0), (500, 0)]},  # 正北7km，V型队形
            2: {'base_position': (-6000, 0), 'formation_offset': [(0, -500), (0, 500)]},  # 正西7km，垂直队形
            3: {'base_position': (5000, -5000), 'formation_offset': [(-500, 500), (500, -500)]}  # 东南7km-5km，斜向队形
        }
    ...
    def done_out_bound(self, agent_id, own_state):
        """出界终止"""
    ...
    def get_reward_assist_attack_enemy(self, last_ally_commands, rewards, ally_states):
        """计算共同击毁敌方的奖励分发"""
        process_rewards = copy.deepcopy(rewards)

        target_to_agents = {}
        for ally_id in range(self.ally_num):
            target = last_ally_commands[ally_id]
            if target not in target_to_agents:
                target_to_agents[target] = []
            target_to_agents[target].append(ally_id)
        ...
        for target, agent_ids in target_to_agents.items():
            ...
            for agent_id in agent_ids:
                agent_state = ally_states[agent_id]
                if agent_state["health_level"] > 0:
                    group_total_reward += self.reward_attack_group[agent_id]
                    group_ally_number += 1

            if group_total_reward > 0:
                if group_ally_number > 0:
                    group_mean_reward = group_total_reward / group_ally_number
                else:
                    group_mean_reward = 0
            elif group_total_reward < 0:
                if group_ally_number > 0:
                    group_mean_reward = group_total_reward * np.sqrt(group_ally_number)
                else:
                    group_mean_reward = 0
            else:
                group_mean_reward = 0

            if group_mean_reward > 1 and group_mean_reward < 6:
                group_mean_reward = 6
            if group_mean_reward < -1 and group_mean_reward > -5:
                group_mean_reward = -6

            for agent_id in agent_ids:
                agent_state = ally_states[agent_id]
                if agent_state["health_level"] > 0:
                    process_rewards[agent_id] += group_mean_reward
                else:
                    process_rewards[agent_id] += 0
    ...
    def get_reward_step_punish(self, agent_id):
        """步数惩罚"""
        step_punish = 0
        step_punish -= self.task_hyper.hyper_reward_step_punish

        return step_punish

    def get_close_target_reward(self, agent_id, own_state, enemy_states):
        """逼近指定目标的奖励"""
        reward_attack_forward_target = 0  # 攻击命令下向指定敌方移动的奖励

        own_pos_vector = own_state["position"]
        own_move_vector = own_state["move_vector"]

        own_command_target_group_id = self.ally_command[agent_id]  # 表示我的目标组别的序号
        ...
        for enemy_id in range(self.enemy_num):
            ...
            if enemy_id_state["health_level"] > 0:
                ...
                own_AO, own_TA, own_R = self.get_AO_TA_R(own_pos_vector, own_move_vector,
                                                         enemy_pos_vector, enemy_move_vector)
                enemy_id_group = self.get_enemy_group(enemy_id)  # 得到该敌机所属的组别
                if own_command_target_group_id == enemy_id_group:
                    # 表示攻击命令并且就是攻击该目标
                    if own_R > self.task_hyper.hyper_feature_close_range:
                        if abs(own_AO) < self.task_hyper.hyper_feature_close_angle:
                            # 向着要攻击的目标前进
                            reward_attack_forward_target += 0.012
                        if abs(own_AO) > self.task_hyper.hyper_feature_remote_angle:
                            # 远离应该攻击的目标
                            reward_attack_forward_target -= 0.012

        return reward_attack_forward_target
                ...
                if abs(own_AO) < self.task_hyper.attack_angle and own_R < self.task_hyper.attack_distance:
                    ...
                    if self.enemy_being_attack[enemy_id] == False and self.ally_launch_attack[agent_id] == False:
                        ...
                        if hit:
                            ...
                            if own_command_target_group_id == enemy_id_group:
                                # 击杀了对应的目标
                                reward_attack_command += 24
                                print("击杀正确")
                            else:
                                reward_attack_command -= 36
                                print("击杀错误")
                            # 击杀之后重新分配组别
                            self.after_attack_assign_command(enemy_id, enemy_id_group)

                            self.reward_attack_group[agent_id] += reward_attack
                            self.reward_attack_group[agent_id] += reward_attack_command
                            ...
                            if own_command_target_group_id == enemy_id_group:
                                # 表示攻击命令下，没能成功击杀
                                reward_attack_command += 0.4
                            else:
                                reward_attack_command -= 0.6

                        self.ally_launch_attack[agent_id] = True

        return reward_attack + reward_attack_command

    def initial_target_dis(self, ally_states, enemy_states):
        """初始化各个智能体与目标组别的距离"""
        self.last_target_dis = [0] * self.ally_num
        for ally_id in range(self.ally_num):
            ally_id_state = ally_states[ally_id]
            ally_id_target_group = self.ally_command[ally_id]
            ally_id_pos_vector = ally_id_state["position"]
            for enemy_id in self.command_groups_running[ally_id_target_group]:
                enemy_id_state = enemy_states[enemy_id]
                enemy_pos_vector = enemy_id_state["position"]
                delta_x, delta_y, delta_z = (enemy_pos_vector[0] - ally_id_pos_vector[0],
                                             enemy_pos_vector[1] - ally_id_pos_vector[1],
                                             enemy_pos_vector[2] - ally_id_pos_vector[2])
                distance = np.linalg.norm([delta_x, delta_y, delta_z])
                self.last_target_dis[ally_id] += distance
