...
from envs.HighEnvSim.envs.dispre_envs_v2.LowEnv_dis_v2 import LowCommandEnv
...
class HighCommandEnv(LowCommandEnv):
    ...
    def __init__(self, high_args, low_args, port, device=torch.device("cpu")):
        ...
        self.max_k_step = 400  # 每一个高级步包含的最大的低级步数
        ...
        # 人类偏好相关变量定义
        self.human_preference = np.zeros((2, 3), dtype=int)  # 人类对元命令的偏好
        self.pre_levels = 4  # 将人类偏好离散化为4个等级，分别为0，1，2，3
        self.human_pre_reward = [[0, 2.5, 3.5, 9], [0, 2.5, 3.5, 9]]  # 对应不同偏好人类给予的奖励
    ...
    def reset(self):
        ...
        # 初始化人类偏好及其编码
        self.human_preference = np.zeros((2, 3), dtype=int)
        self.human_preference[0][2] = 3
        # self.human_preference[1][0] = 3
    ...
    def real_time_human_prefer(self):
        ...
        实时修改人类对元命令的偏好的接口
        """
        human_preference = np.zeros((2, 3), dtype=int)
        for i in range(self.enemy_group_num):
            random_action = np.random.randint(0, 2)
            human_preference[random_action][i] = np.random.randint(0, self.pre_levels)
    ...
    def get_reward_human_prefer(self, agent_id, actions):
        ...
        if own_action == 0:
            ...
            reward_attack_prefer = attack_preference * self.human_pre_reward[own_action][attack_preference]
            escape_preference = self.human_preference[1][own_target]
            reward_escape_prefer = -(escape_preference * self.human_pre_reward[1][escape_preference])
            ...
            if own_in_target_danger_range:
                ...
                reward_human_prefer = preference * self.human_pre_reward[1][preference]
            else:
    ...
    def encode_human_preference(self):
        ...
        for group_id in range(self.enemy_group_num):
            if action_mask[0][group_id] == 0:
                ...
                attack_prefer_encode_feats[group_id] = (attack_preference + 1) / self.pre_levels  # 将偏好归一化到[0.25, 0.5, 0.75, 1]

                escape_preference = self.human_preference[1][group_id]
                escape_prefer_encode_feats[group_id] = (escape_preference + 1) / self.pre_levels
    ...
    def one_hot_encode_preference(self, preference):
        """将人类的偏好编码为独热编码"""
        one_hot_encoded = np.zeros(self.pre_levels)
        one_hot_encoded[preference] = 1
                ...
                for group_id in human_prefer_escape_id:
                    if ally_in_target_range[group_id][ally_id] == 0:
                        ...
                        if is_from_safe_to_danger:
                            ...
                            reward[ally_id] -= (preference * self.human_pre_reward[1][preference])
