...
class ReplayBuffer(Buffer):
    ...
    def __init__(self, args, num_agents, obs_space, act_space):
        ...
        self.action_mask = np.ones((self.buffer_size + 1, self.n_rollout_threads, self.num_agents, 3), dtype=np.float32)
        self.step = 0
    ...
    def insert(self,
        ...
        self.action_mask[self.step + 1] = action_mask.copy()
    ...
    def after_update(self):
        ...
        self.action_mask[0] = self.action_mask[-1].copy()
    ...
    def clear(self):
        ...
        self.rnn_states_critic = np.zeros_like(self.rnn_states_actor)
        self.action_mask = np.ones_like(self.action_mask)
...
class SharedReplayBuffer(ReplayBuffer):
    ...
    def insert(self,
        ...
        if available_actions is not None:
            pass
        return super().insert(obs, actions, rewards, masks, action_log_probs, value_preds, rnn_states_actor, rnn_states_critic, action_mask)
    ...
    def recurrent_generator(self, advantages: np.ndarray, num_mini_batch: int, data_chunk_length: int):
        ...
        action_mask = self._cast(self.action_mask[:-1])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                obs_batch.append(obs[ind:ind + data_chunk_length])
                action_mask_batch.append(action_mask[ind:ind + data_chunk_length])
            ...
            # These are all from_numpys of size (L, N, Dim)
            obs_batch = np.stack(obs_batch, axis=1)
            action_mask_batch = np.stack(action_mask_batch, axis=1)
            ...
            action_mask_batch = self._flatten(L, N, action_mask_batch)
            ...
            yield obs_batch, share_obs_batch, actions_batch, masks_batch, active_masks_batch, \
                old_action_log_probs_batch, advantages_batch, returns_batch, value_preds_batch, \
                rnn_states_actor_batch, rnn_states_critic_batch, action_mask_batch
