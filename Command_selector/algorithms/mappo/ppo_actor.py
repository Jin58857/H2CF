...
class PPOActor(nn.Module):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        # 临时变量定义
        self.own_feat_dim = 25
        self.ally_feat_dim = 19
    ...
    def forward(self, obs, rnn_states, masks, action_mask, deterministic=False):
        obs = check(obs).to(**self.tpdv)
        rnn_states = check(rnn_states).to(**self.tpdv)
        masks = check(masks).to(**self.tpdv)
        action_mask = check(action_mask).to(**self.tpdv)
        ...
        actions, action_log_probs = self.act(actor_features, action_mask, deterministic)

        return actions, action_log_probs, rnn_states

    def evaluate_actions(self, obs, rnn_states, action, masks, action_mask, active_masks=None):
        obs = check(obs).to(**self.tpdv)
        rnn_states = check(rnn_states).to(**self.tpdv)
        ...
        action_log_probs, dist_entropy = self.act.evaluate_actions(actor_features, action, action_mask, active_masks)

        return action_log_probs, dist_entropy

    def forward_2(self, obs, rnn_states, masks, action_mask, deterministic=False):
        obs = check(obs).to(**self.tpdv)
        rnn_states = check(rnn_states).to(**self.tpdv)
        masks = check(masks).to(**self.tpdv)
        action_mask = check(action_mask).to(**self.tpdv)
