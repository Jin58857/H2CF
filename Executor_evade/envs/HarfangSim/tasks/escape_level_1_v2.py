...
class MultiCombatTask(object):
    ...
    def load_observation_space(self):
        """ 指定观测空间、共享观测空间"""
        self.obs_length = 17 + 1 * 14 + self.enemy_num * 18
    ...
    def get_obs(self, agent_id, ally_states, enemy_states):
        """
        与躲避相关的观测信息
        自身信息：位置、速度、姿态、健康值、自己要躲避的目标
        离自己最近的队友信息：相对位置、速度、距离、角度关系
        敌方信息：敌方组别是否为目标的标志
        """
        ...
        if own_state["health_level"] > 0:
            ...
            for enemy_id in range(self.enemy_num):
                ...
                if enemy_id_state["health_level"] > 0:
                    ...
                    # 该敌方是否为自己目标的标志
                    if own_target_group == enemy_id_group:
                        is_own_target = 1
                    else:
                        is_own_target = 0
                    ...
                    enemy_feats[enemy_id, 0:18] = np.array([norm_enemy_relative_x, norm_enemy_relative_y,
                                                           ...
                                                           np.cos(enemy_HA), norm_enemy_R, health_level, is_own_target] +
    ...
    def get_reward(self, agent_id, ally_states, enemy_states):
        ...
        if own_state["health_level"] > 0:
            ...
            reward_to_safe_range = self.get_reward_to_safe_range(agent_id, ally_states, enemy_states)  # 远离目标组别的稠密奖励
            ...
            #       "reward_to_safe_range:", reward_to_safe_range,
            #       "reward_attack:", reward_attack)

            own_reward = (1.0 * reward_out_bound + 1.0 * reward_safe_distance
                          + 1.0 * reward_danger_range + 1.0 * reward_to_safe_range + 1.0 * reward_attack)

        return own_reward

    def get_reward_to_safe_range(self, agent_id, ally_states, enemy_states):
        """远离目标组别的启发式奖励"""
        reward = 0
        current_min_dis = np.inf
        ...
        # 检查是否存在上一次的最小距离
        if self.last_target_min_dis[agent_id] != np.inf:
            delta_dis = current_min_dis - self.last_target_min_dis[agent_id]
            epsilon = 1e-6  # 防止除以零
            base_reward = (delta_dis) / (self.last_target_min_dis[agent_id] + epsilon)
            base_reward = np.clip(base_reward, -10, 10)  # 根据需要调整范围

            if current_min_dis < self.task_hyper.danger_range:
                # 在危险区域内，奖励或惩罚更大
                reward = base_reward * 20  # 需要debug调整
            else:
                # 在安全区域内，奖励或惩罚较小
        ...
        # 更新上一次的最小距离
        self.last_target_min_dis[agent_id] = current_min_dis
    ...
    def get_reward_safe_distance(self, agent_id, own_state, ally_states, enemy_states):
        ...
        # 与最近队友的安全距离惩罚
        nearst_ally_id, current_min_ally_dis = self.fine_nearst_ally(agent_id, ally_states)
        if nearst_ally_id != -1:
            if current_min_ally_dis < self.task_hyper.hyper_feature_safe_distance:
    ...
    def initial_target_min_dis(self, ally_states, enemy_states):
        """初始化与所选组别的最小距离"""

        for ally_id in range(self.ally_num):
            ally_state = ally_states[ally_id]
            ally_x, ally_y, ally_z = ally_state["position"]
            ally_target_group = self.ally_command[ally_id]

            for enemy_id in self.command_groups_running[ally_target_group]:
                enemy_state = enemy_states[enemy_id]
                if enemy_state["health_level"] > 0:
                    enemy_x, enemy_y, enemy_z = enemy_state["position"]
                    relative_pos = np.array([enemy_x - ally_x, enemy_y - ally_y, enemy_z - ally_z])
                    distance = np.linalg.norm(relative_pos)
                    if distance < self.last_target_min_dis[ally_id]:
                        self.last_target_min_dis[ally_id] = distance
